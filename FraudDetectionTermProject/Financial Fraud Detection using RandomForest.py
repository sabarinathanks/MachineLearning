...
# MAGIC %md # Financial Fraud Detection using Random Forest Classifier
# MAGIC 
...
# MAGIC %md ### Calculate Differences between Originating and Destination Balanaces
...
# MAGIC %md ## Rules-based Model: Create a set of rules to identify fraud based on known cases
# MAGIC 
...
oldbalance_limit=100000
newbalancedest_limit=250
newbalance_orig=20
...
#df = df.withColumn("label", 
 #                  F.when(
  #                   (
   #                    (df.oldbalanceOrg <= oldbalance_limit) & (df.type == "TRANSFER") & (df.newbalanceDest <= newbalancedest_limit)) | 
    #                   (
     #                    (df.oldbalanceOrg > oldbalance_limit) & (df.newbalanceOrig <= newbalance_orig)) | 
      #                     (
       #                      (df.oldbalanceOrg > oldbalance_limit) & (df.newbalanceOrig > newbalance_orig) & (df.amount > amount_limit)
        #                   ), 1
         #          ).otherwise(0))
...
# MAGIC %md ### Create ML Pipeline - Using Random Forest Classifier
...
dt = RandomForestClassifier(labelCol = "label", featuresCol = "features", seed = 54321, maxDepth = 5)
...
# MAGIC %md ## View RandomTree Model
